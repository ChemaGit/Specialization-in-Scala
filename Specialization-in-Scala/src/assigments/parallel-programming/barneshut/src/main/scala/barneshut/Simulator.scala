...
class Simulator(val taskSupport: TaskSupport, val timeStats: TimeStatistics) {
  ...
  def updateBoundaries(boundaries: Boundaries, body: Body): Boundaries = {
    boundaries.maxX = if(body.x > boundaries.maxX) body.x else boundaries.maxX
    boundaries.maxY = if(body.y > boundaries.maxY) body.y else boundaries.maxY
    boundaries.minX = if(body.x < boundaries.minX) body.x else boundaries.minX
    boundaries.minY = if(body.y < boundaries.minY) body.y else boundaries.minY
  ...
  def mergeBoundaries(a: Boundaries, b: Boundaries): Boundaries = {
    ...
    nBoundaries.minX = if(a.minX < b.minX) a.minX else b.minX    
    nBoundaries.minY = if(a.minY < b.minY) a.minY else b.minY    
    nBoundaries.maxX = if(a.maxX > b.maxX) a.maxX else b.maxX    
    nBoundaries.maxY = if(a.maxY > b.maxY) a.maxY else b.maxY    
  ...
  def computeSectorMatrix(bodies: Seq[Body], boundaries: Boundaries): SectorMatrix = timeStats.timed("matrix") {
    val parBodies = bodies.par
    parBodies.tasksupport = taskSupport
    val sMatrix = new SectorMatrix(boundaries, SECTOR_PRECISION)
    parBodies.aggregate(sMatrix)(_.+=(_), _.combine(_))
    sMatrix
  }
  ...
  def updateBodies(bodies: Seq[Body], quad: Quad): Seq[Body] = timeStats.timed("update") {
    val parBodies = bodies.par
    parBodies.tasksupport = taskSupport
    parBodies.map(f => f.updated(quad)).seq
  }
