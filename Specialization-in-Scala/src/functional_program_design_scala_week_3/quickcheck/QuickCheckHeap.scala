package functional_program_design_scala_week_3.quickcheck
...
object myApp extends Properties("Heap") {

  class QuickCheckHeap extends IntHeap with BinomialHeap {
    ...
    property("sorted_sequence") = forAll { (h: H) =>
      ...
      //println(lSorted.mkString(","))
      //println(lSorted.sorted.mkString(","))
    ...
    def prove(z: H): H = {
      z match {
        case t :: ts => print(t.x + ","); prove(ts)
        case empty => println("***********"); z  
      }        
    }    
    
    
    property("prove") = forAll { (x: H) =>
      isEmpty(prove(x))
    }    
    */
