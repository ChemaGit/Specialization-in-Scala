...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {    
    ...
    if(!chars.isEmpty) go(chars.head, chars.tail, List())
    else List()
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def go(tre: CodeTree, head: Bit, tail: List[Bit], acc: List[Char]): List[Char] = {           
      ...
      else go(tree, lBits.head, lBits.tail, ch :: acc)
        
    }   
    if(!bits.isEmpty) go(tree, bits.head, bits.tail, List())
    else List()
  }
  ...
  def quickEncode(tree: CodeTree)(text: List[Char]): List[Bit] = {    
    def go(listCh: List[Char], ct: CodeTable): List[Bit] = {
      ...
      else go(listCh.tail,ct) ::: codeBits(ct)(listCh.head).reverse
    }           
    if(text.isEmpty) List()
    else go(text, convert(tree))
  }
}
