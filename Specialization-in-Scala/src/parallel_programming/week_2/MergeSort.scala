...
object MergeSort {
  ...
  // a bit of reflection to access the private sort1 method, which takes an offset an argument
  private val sort = {
    val method = scala.util.Sorting.getClass.getDeclaredMethod("sort", classOf[Array[Int]], classOf[Int], classOf[Int])
  ...
  def parMergeSort(xs: Array[Int], maxDepth: Int): Unit = {
    ...
    //~46x less time than a full quickSort without GCs(average time)
    //Therefore
    ...
    //Without the merge step, the sort phase parallelizes almost lineraly.
    ...
    //~16x less time than a full quickSort without GCs(average time)
