...
object Interaction {
  ...
  def generateImage(year: Int,
                    tile: Tile,
                    inputs: Iterable[(Location, Temperature)]): Unit = {
    ...
    Interaction.tile(inputs,points, tile).output(new java.io.File("target/temperatures/" + year + "/" + tile.zoom + "/" + tile.x + "-" + tile.y + ".png"))
    println("target/temperatures/" + year + "/" + tile.zoom + "/" + tile.x + "-" + tile.y + ".png")
  }  
  ...
  /*def tile(temperatures: Iterable[(Location, Temperature)], colors: Iterable[(Temperature, Color)], tile: Tile): Image = {
    val coordinates: Seq[(Int, Int)] = for {
      ...
      (if (i == 0) tile.x else tile.x + i / IMAGE_WIDTH,
        if (j == 0) tile.y else tile.y + j / IMAGE_HEIGHT)
    }
    
    val arrPixel = coordinates.par.map({case(x, y) => Tile(x, y, tile.zoom).toLocation})
                              .map({case(location) => interpolateColor(colors, predictTemperature(temperatures, location))})
                              .map({case(color) => Pixel(color.red, color.green, color.blue, ALPHA)}).toArray
    Image(IMAGE_WIDTH, IMAGE_HEIGHT, arrPixel)                              
  }*/   
  ...
  /*def tile(temperatures: Iterable[(Location, Temperature)], colors: Iterable[(Temperature, Color)], tile: Tile): Image = {
    val coordinates = 0 until IMAGE_WIDTH * IMAGE_HEIGHT
                          
    val arrPixel = coordinates.par.map({case(coord) => ((coord % IMAGE_WIDTH).toDouble / IMAGE_WIDTH + tile.x, (coord / IMAGE_HEIGHT).toDouble / IMAGE_HEIGHT + tile.y)})
                              .map({case(x, y) => interpolateColor(colors, predictTemperature(temperatures, Tile(x.toInt, y.toInt, tile.zoom).toLocation))})                              
                              .map({case(color) => RGBColor(color.red, color.green, color.blue, ALPHA).toPixel}).toArray
    Image(IMAGE_WIDTH, IMAGE_HEIGHT, arrPixel)                           
  }*/  
  
  /*def tile(temperatures: Iterable[(Location, Temperature)], colors: Iterable[(Temperature, Color)], tile: Tile): Image = {        
    val arrPixels = (0 until IMAGE_HEIGHT * IMAGE_WIDTH).map(index => {

      val x = (index % IMAGE_WIDTH).toDouble / IMAGE_WIDTH + tile.x 
      val y = (index / IMAGE_HEIGHT).toDouble / IMAGE_HEIGHT + tile.y 

      val color  = interpolateColor(colors,predictTemperature(temperatures,Tile(x.toInt, y.toInt, tile.zoom).toLocation))
      
      Pixel(color.red, color.green,color.blue, ALPHA)
      
      }).seq.toArray
    Image(IMAGE_WIDTH, IMAGE_HEIGHT, arrPixels)
  }*/    
}
