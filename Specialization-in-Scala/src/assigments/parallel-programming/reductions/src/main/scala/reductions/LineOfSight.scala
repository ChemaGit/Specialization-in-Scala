...
object LineOfSight {
  ...
  def lineOfSight(input: Array[Float], output: Array[Float]): Unit = {    
    ...
    while(from < until) {
      output(from) = max(input(from) / from, output(from - 1))
      from += 1
    }
  }
  ...
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
    ...
    var maxAngle = 0f
    while(f < until) {      
      maxAngle = max(maxAngle, input(f) / f)      
      f += 1
    }  
    maxAngle
  }
  ...
  def downsweepSequential(input: Array[Float], output: Array[Float],
    ...
    while(f < until) {
      maxAngle =  max(input(f) / f, maxAngle)
      output(f) = maxAngle
      f += 1     
    }   
  }
  ...
  def downsweep(input: Array[Float], output: Array[Float], startingAngle: Float,
    tree: Tree): Unit = {
    tree match {
      case Leaf(from, end, angle) => downsweepSequential(input, output, startingAngle, from , end)
