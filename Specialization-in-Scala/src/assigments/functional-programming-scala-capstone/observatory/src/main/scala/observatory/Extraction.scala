...
object Extraction {
  ...
  /**
   * We are going to process the huge files bit by bit.
   */
  def getPartitionLocateTemperatures(rdd: RDD[(SelectClass, Long)]): Iterable[(LocalDate, Location, Temperature)] = {
    rdd.persist(DISK_ONLY)
    @annotation.tailrec
    def loop(from: Int, to: Int, aux: Iterable[(LocalDate, Location, Temperature)]): Iterable[(LocalDate, Location, Temperature)] = {
      if(from < 0){
        rdd.unpersist()
        aux
      }
      else {
        val temp = rdd.filter({case(value, index) => index >= from && index < to})
                      .map({case(value, index) => value}).collect().par
                      .map(j => (LocalDate.of(j.year,j.month,j.day),j.loc,j.temperature)).toVector
        if(temp.isEmpty) loop(-1, to, aux)
        else {
          loop(to, to + 500000, aux ++: temp)          
        }
      }
    }      
    loop(0,500000, Vector())
  }
  ...
  def locateTemperatures(year: Year, stationsFile: String, temperaturesFile: String): Iterable[(LocalDate, Location, Temperature)] = { 
    ...
    //.map(j => (LocalDate.of(j.year,j.month,j.day),j.loc,j.temperature)).toVector          
    val lTempRDD = locateTemperaturesSpark(year, stationsFile, temperaturesFile).rdd.zipWithIndex()    
    getPartitionLocateTemperatures(lTempRDD)
   ...
   * We are going to process the huge files bit by bit.
   */  
  def getPartitionLocateAverage(rdd: RDD[((Location, Temperature), Long)]): Iterable[(Location, Temperature)] = {
    rdd.persist(DISK_ONLY)
    @annotation.tailrec
    def loop(from: Int, to: Int, aux: Iterable[(Location, Temperature)]): Iterable[(Location, Temperature)] = {
      if(from < 0){
        rdd.unpersist()
        aux
      }
        ...
        val temp = rdd.filter({case(value, index) => index >= from && index < to})
                      .map({case(value, index) => value}).collect().par.toVector                      
        if(temp.isEmpty) loop(-1, to, aux)
        else {
          loop(to, to + 1000, aux ++: temp)          
        }        
      }     
    }
    loop(0, 1000, Vector())
  ...
  /*def locationYearlyAverageRecords(records: Iterable[(LocalDate, Location, Temperature)]): Iterable[(Location, Temperature)] = { 
    ...
    val temp = locationYearlyAverageRecordsSpark(rdd).zipWithIndex()
    val res = getPartitionLocateAverage(temp)
    spark.sparkContext.stop()
    res
  }*/  
